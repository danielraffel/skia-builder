...
if [ "$1" = "api" ] && [ "$2" = "repos/owner/repo/actions/runs/777" ]; then
  cat <<'JSON'
{
  "id": 777,
  "status": "in_progress",
...
if [ "$1" = "api" ] && [ "$2" = "repos/owner/repo/actions/runs/777/jobs?per_page=100" ]; then
  cat <<'JSON'
{
  "jobs": [
    {
      "name": "build-skia (macos-15, ios, gpu, arm64,x86_64, simulator)",
      "status": "in_progress",
      "conclusion": null,
      "created_at": "2026-07-06T16:52:47Z",
      "started_at": "2026-07-06T16:52:47Z",
      "html_url": "https://github.com/owner/repo/actions/runs/777/job/1"
    },
    {
      "name": "build-skia (ubuntu-latest, linux, gpu, x64)",
      "status": "completed",
      "conclusion": "success",
      "created_at": "2026-07-06T16:52:47Z",
      "html_url": "https://github.com/owner/repo/actions/runs/777/job/2"
    }
  ]
}
JSON
...
running_output="$(
  ...
  WATCH_RUN_ID=777 \
  SKIA_BRANCH=chrome/m151 \
  ACTIVE_STALE_MINUTES=240 \
  NOW_EPOCH=9999999999 \
  bash "${SCRIPT_DIR}/report-build-progress.sh"
)"

if [[ "$running_output" != *"stale in-progress matrix job for at least 240 minutes"* ]]; then
  echo "FAIL: missing stale in-progress matrix detection" >&2
  echo "$running_output" >&2
  exit 1
fi

if [[ "$running_output" != *"build-skia (macos-15, ios, gpu, arm64,x86_64, simulator)"* ]]; then
  echo "FAIL: missing stale running iOS simulator job" >&2
  echo "$running_output" >&2
  exit 1
fi

if [[ "$running_output" != *"in progress for"* ]]; then
  echo "FAIL: missing in-progress age" >&2
  echo "$running_output" >&2
  exit 1
fi
